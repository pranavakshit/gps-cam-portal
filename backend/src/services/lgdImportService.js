...
exports.processLgdZip = processLgdZip;
...
async function processLgdZip(buffer, onProgress, isCancelled) {
    if (onProgress)
        onProgress(0, 'Reading ZIP archive...');
    const zip = new adm_zip_1.default(buffer);
    const zipEntries = zip.getEntries();
    zipEntries.sort((a, b) => {
        const getPriority = (name) => {
            const lower = name.toLowerCase();
            if (lower.includes('villageofspecificstate'))
                return 3;
            if (lower.includes('subdistrict'))
                return 2;
    ...
    for (let i = 0; i < validEntries.length; i++) {
        ...
        if (!entry)
            continue;
        if (onProgress)
            onProgress((i / validEntries.length) * 5, `Analyzing file size: ${entry.name}...`);
    ...
    for (let i = 0; i < validEntries.length; i++) {
        ...
        const reportRowProgress = () => {
            ...
            if (onProgress && absoluteProcessedRows % 100 === 0) {
                ...
                onProgress(Math.min(99, progress), `Parsing ${entryName} (${absoluteProcessedRows.toLocaleString()} / ${absoluteTotalRows.toLocaleString()}) - ${speed} rows/sec - ETA: ${etaSecs}s`);
            }
        };
        if (lowerName.includes('villageofspecificstate')) {
            ...
            if (headerIdx !== -1) {
                ...
                for (let j = headerIdx + 2; j < data.length; j++) {
                    ...
                    reportRowProgress();
                    const row = data[j];
                    if (!row || !row[3] || isNaN(parseInt(row[3])))
                        continue;
                    const districtCode = parseInt(row[1]);
                    const districtName = row[2] || `District ${districtCode}`;
    ...
    if (onProgress)
        onProgress(100, 'Database sync complete.');
}
//# sourceMappingURL=lgdImportService.js.map