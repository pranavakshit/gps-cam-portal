...
exports.hardDeletePhoto = exports.restorePhoto = exports.deletePhoto = exports.abortDeletePhoto = exports.completeDeletePhoto = exports.rejectDeletePhoto = exports.approveDeletePhoto = exports.requestDeletePhoto = exports.getPhotos = exports.uploadPhoto = exports.upload = void 0;
...
const uploadPhoto = async (req, res) => {
    try {
        ...
        const { stateCode, districtCode, subdistrictCode, villageCode, ulbCode, wardCode, locationName, latitude, longitude, timestamp, uploader } = req.body;
        ...
        let user = await prisma_1.default.user.findUnique({ where: { username: uploader } });
        if (!user) {
...
const getPhotos = async (req, res) => {
    try {
        ...
        const transformed = photos.map(p => {
            ...
            const locParts = [];
            if (p.villageCode)
                locParts.push(`Village ${p.villageCode}`);
            if (p.wardCode)
                locParts.push(`Ward ${p.wardCode}`);
            if (p.subdistrictCode)
                locParts.push(`SubDist ${p.subdistrictCode}`);
            if (p.ulbCode)
                locParts.push(`ULB ${p.ulbCode}`);
            if (p.districtCode)
                locParts.push(`Dist ${p.districtCode}`);
            if (p.stateCode)
                locParts.push(`State ${p.stateCode}`);
        ...
        if (expiredPhotoIds.length > 0) {
            prisma_1.default.photo.updateMany({
...
const requestDeletePhoto = async (req, res) => {
    try {
        const { id } = req.params;
        const { reason } = req.body;
        const photo = await prisma_1.default.photo.findUnique({ where: { id: Number(id) } });
        if (!photo) {
...
exports.requestDeletePhoto = requestDeletePhoto;
const approveDeletePhoto = async (req, res) => {
    try {
        const { id } = req.params;
        const photo = await prisma_1.default.photo.findUnique({ where: { id: Number(id) } });
        if (!photo) {
...
exports.approveDeletePhoto = approveDeletePhoto;
const rejectDeletePhoto = async (req, res) => {
    try {
        const { id } = req.params;
        const photo = await prisma_1.default.photo.findUnique({ where: { id: Number(id) } });
        if (!photo) {
...
exports.rejectDeletePhoto = rejectDeletePhoto;
const completeDeletePhoto = async (req, res) => {
    try {
        const { id } = req.params;
        const photo = await prisma_1.default.photo.findUnique({ where: { id: Number(id) } });
        if (!photo) {
...
exports.completeDeletePhoto = completeDeletePhoto;
const abortDeletePhoto = async (req, res) => {
    try {
        const { id } = req.params;
        const photo = await prisma_1.default.photo.findUnique({ where: { id: Number(id) } });
        if (!photo) {
...
exports.abortDeletePhoto = abortDeletePhoto;
...
exports.restorePhoto = restorePhoto;
const hardDeletePhoto = async (req, res) => {
    try {
        const { id } = req.params;
        ...
        const filePath = path_1.default.join(__dirname, '../../', photo.imageUrl);
...
exports.hardDeletePhoto = hardDeletePhoto;
